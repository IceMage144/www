...
function parse (string, den) {
    ...
    while (i < string.length) {
        ...
        if (results[0]) {
            ...
            print("Some error ocurred on parsing");
            break;
        }
...
function verifySintax (array) {
    ...
    while (i < array.length) {
        ...
        if (array[i].type == "Function") {
            ...
            if (i + 1 >= array.length || array[i+1].value != "(") {
                console.log("expected parenthesis");
                return 0;
            }
            par = 1;
    ...
    i = 0;
    if (commas != 0) {
        print("too many/few commas");
        return 0;
    }
    while (i < array.length) {
        if (array[i].value == ",") {
            print("Comma out of function");
            return 0;
        }
    ...
    while (i < array.length) {
        ...
        if (i < array.length - 1  && i > 0 && array[i].type == "Operator") {
            if (array[i+1].type == "Operator" || array[i+1].type == ")" || array[i+1].type == "," || array[i-1].type == "Operator" || array[i-1].type == "(" || array[i-1].type == ",") {
                print("Operator without operands");
                return 0;
            }
        }
        ...
        if (i < array.length - 1 && i > 0 && (array[i].type == "Variable" || array[i].type == "Constant" || array[i].type == "Number")) {
            if (array[i+1].type == "Constant" || array[i+1].type == "Number" || array[i+1].type == "Number" || array[i-1].type == "Constant" || array[i-1].type == "Variable" || array[i-1].type == "Number") {
                print("Numbers without operator");
                return 0;
            }
        }
...
function derivateProduct (array) {
    ...
    while (num < 2) {
        if (array[i].type == "Function") {
            ...
            //print("3");
            var fact = getFactor(array, i + 1) + i + 2;
            while (i < fact) {
                factors[num].push(array[i]);
                i += 1;
            }
...
function derivateFunct (array) {
    ...
    if (array[0].value == "pow") {
        ...
        print("Program does not sopport " + array[0].value + " yet");
    }
...
function main () {
    ...
    if (res.bool) {
        ...
        if (!verifySintax(array)) {
            nestedDiv.textContent = "Invalid sintax";
        }
        else {
            toPrefix(array);
            //print(ArraytoString(array));
        ...
        nestedDiv.textContent = "Invalid variable";
    }
}
